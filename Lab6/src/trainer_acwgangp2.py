            ...
            with tqdm(total=len(self.train_loader), ncols=100) as pbar:
                ...
                while iterator < len(self.train_loader):
                    ...
                    while d_iter < self.diter and iterator < len(self.train_loader):
                        ...
                        aux_loss = nn.functional.binary_cross_entropy(
                    ...
                    self.eval_model.resnet18.eval()
                    pred_gen_labels_e = self.eval_model.resnet18(fake_images)

                    aux_loss = (1 - self.aux_mix) * nn.functional.binary_cross_entropy(pred_gen_labels, right_label)
                    aux_loss += self.aux_mix * nn.functional.binary_cross_entropy(pred_gen_labels_e, right_label)
                    g_loss = -g_loss.mean() + self.lambda_ac_g * aux_loss
                    g_loss.backward()
                    self.optimG.step()
                    pbar.set_postfix_str(f'loss D:{d_loss:.4f}, G:{g_loss:.4f}')
                    pbar.update(0)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--ac_d", default=1, type=float)
    parser.add_argument("--ac_g", default=1, type=float)
    parser.add_argument("--aux_mix", default=0.5, type=float)
    parser.add_argument('--resume', default=False, action='store_true', help='resume training')
    parser.add_argument("--save_path", default='../ckpt')
    parser.add_argument("--run_name", default='acwgangp2')
    ...
    if not args.inference:
        ...
        trainer.generator.load_state_dict(torch.load('../ckpt/acwgangp4_ac0.5/gen_best.pth'))
