...
class Trainer():
    ...
    def test_both_save(self, epoch):
        test_score, grid1, grid1_seq = self.test(self.test_loader)
        test_new_score, grid2, grid2_seq = self.test(self.new_test_loader)
        if (epoch+1) % 10 == 0:
            ...
            # path3 = os.path.join(self.args.result_root, args.run_name, f"process_{epoch}.png")
        else:
            ...
            # path3 = os.path.join(self.args.result_root, args.run_name, f"process.png")
        save_image(grid1, path1)
        save_image(grid2, path2)
        # save_image(grid1_seq, path3)
    ...
    def test(self, test_loader):
        ...
        with torch.no_grad():
            for cond in test_loader:
                ...
                x_i, _ = self.ddpm.sample(cond, (3, 64, 64), self.device)
                x_gen.append(x_i)
                label.append(cond)
            ...
            # "red sphere"=9, "yellow cube"=7, "cyan cylinder"=22
            cond = torch.zeros((1, 24), device=self.device)
            cond[0, 9], cond[0, 7], cond[0, 22] = 1, 1, 1
            _, x_seq = self.ddpm.sample(cond, (3, 64, 64), self.device)
            grid_seq = make_grid(x_seq, nrow=5, normalize=True, scale_each=True)
...
if __name__ == '__main__':
    ...
    if not args.inference:
        ...
        # path = os.path.join(args.model_root, args.run_name, "linear/model_test2_314.pth")
        path = os.path.join(args.model_root, args.run_name, "linear/model_best.pth")
        # path = os.path.join(args.model_root, args.run_name, "linear/model_latest_train.pth")
        trainer.ddpm.load_state_dict(torch.load(path))
        ...
        # path = os.path.join(args.model_root, "model_test_new.pth")
        # trainer.ddpm.load_state_dict(torch.load(path))
